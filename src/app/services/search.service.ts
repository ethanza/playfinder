...
export default class SearchService {
    ...
    public getSpecificSlot(api_params: ApiParams): void {
        this.subscription = this.http.get<Result>('assets/slots.json').subscribe((result) => {
            const specificResult = result.data.find(slot => parseInt(slot.id) == api_params.pitch_id) 
            result.data = [];
    ...
    public getAllAvailableSlots(): void {
        this.subscription = this.http.get<Result>('/assets/slots.json').subscribe((result) => {
            const availableSlots = result.data.filter(slot => slot.attributes.availabilities > 0);
